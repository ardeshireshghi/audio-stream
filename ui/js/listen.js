(() => {
  ...
  function init() {
    ...
    const renderTracks = async (tracks) => {
      ...
      tracks.forEach((track) => {
        ...
        const renderedTemplate = trackTemplate.innerHTML
          .replace(
            '{{name}}',
            track.name.replace(/-/g, ' ').replace('.ogg', '')
          )
    ...
    // Set the page cover and title
    document.querySelector(
      '.js-cover'
    ).style.backgroundImage = `url(${config.listenPage.coverImageUrl})`;

    document.querySelector('.js-page-title').textContent =
      config.listenPage.title;
  }
